...
const ChatHeader = () => {
  ...
  const { handleChats, handleContacts } = useChatStore();
  ...
  return (
    <>
      <div className="flex font-orbitron justify-center items-center h-screen     flex-col">
        ...
        <div className="shadow-2xl shadow-blue-950 p-3 rounded-sm h-72 bg-gray-800 text-white mt-1 overflow-y-scroll  overflow-x-hidden hide-scrollbar">
          ...
          <div className="flex border-t-2 border-white justify-center gap-15 w-80 mt-5">
            <div
              onClick={handleChats}
              className="bg-cyan-600 px-5 rounded-sm text-white py-1 mt-2 font-semibold text-sm cursor-pointer"
            >
              Chats
            </div>
          ...
          {activeTab == "contacts" ? <Contacts /> : <ChatPage />}
        </div>
      </div>
    </>
  );
